  ...
  switch (parsed.family) {
    ...
    case "route": {
      if (op === "add") {
        ...
        // A description is free text that ships into device state; refuse
        // command-shaped content there just as for preference values.
        if (
          parsedValue.data.description !== null &&
          looksExecutable(parsedValue.data.description)
        ) {
          throw invalidValue(path, "route description contains executable content");
        }
        const route: Route = {
