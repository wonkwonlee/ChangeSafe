...
/**
 * Turn already-decoded manifest documents into deterministic add/replace
 * operations. This module intentionally has no YAML parser dependency so
 * typed in-memory fixtures can use the same production derivation boundary.
 * Omission is deliberately not deletion, and every inverse is captured
 * mechanically.
 */
