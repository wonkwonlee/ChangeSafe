...
export function KubernetesWorkbenchShell({
  ...
  return <div className="min-h-screen bg-canvas text-ink">
    ...
    <div id="review" className="mx-auto grid max-w-[1600px] grid-cols-1 gap-4 px-4 py-5 sm:px-6 xl:grid-cols-[minmax(220px,0.75fr)_minmax(0,2fr)_minmax(280px,0.95fr)]">
      <aside aria-label="Kubernetes review context" className="min-w-0 rounded-xl border border-edge bg-surface p-4"><Label>Kubernetes examples</Label><h1 className="mt-2 text-lg font-semibold">{fixture.label}</h1><ul className="mt-4 grid gap-2" role="list" aria-label="Bundled Kubernetes examples">{KUBERNETES_REVIEW_EXAMPLES.map((candidate) => <li key={candidate.sourceId}><button aria-pressed={candidate.sourceId === selectedSourceId} className="w-full rounded border border-edge px-3 py-2 text-left text-sm text-ink-dim hover:border-active focus:outline-none focus:ring-2 focus:ring-active disabled:cursor-wait" disabled={workflow.phase === "ANALYZING"} onClick={() => selectExample(candidate.sourceId)} type="button"><span className="block font-medium text-ink">{candidate.label}</span><span className="mt-1 block text-xs">{candidate.description}</span></button></li>)}</ul><section id="sources" className="mt-5 border-t border-edge pt-4" aria-labelledby="source-title"><h2 id="source-title" className="text-sm font-semibold">Offline source</h2><dl className="mt-3 grid gap-3 text-xs"><div><dt className="text-ink-faint">Snapshot ID</dt><dd className="mt-1 font-mono">{KUBERNETES_PUBLIC_REPLAY_SNAPSHOT.snapshotId}</dd></div><div><dt className="text-ink-faint">Namespace</dt><dd className="mt-1">{KUBERNETES_PUBLIC_REPLAY_SNAPSHOT.provenance.namespaces.join(", ")}</dd></div><div><dt className="text-ink-faint">Fixture provenance</dt><dd className="mt-1">{fixture.provenance}</dd></div><div><dt className="text-ink-faint">Policy version</dt><dd className="mt-1 font-mono">{example.session.policyVersion}</dd></div></dl></section></aside>
      <main aria-busy={workflow.phase === "ANALYZING"} aria-label="Kubernetes review canvas" className="min-w-0 rounded-xl border border-edge bg-surface p-4 sm:p-6"><header className="flex flex-wrap items-start justify-between gap-4 border-b border-edge pb-5"><div><Label>Offline replay evaluation</Label><h2 className="mt-2 text-xl font-semibold">{workflow.phase}</h2><p className="mt-2 max-w-2xl text-sm leading-relaxed text-ink-dim"><StateValue state={workflow} /></p><p aria-atomic="true" aria-live="polite" className="sr-only" role="status"><ReplayStatus state={workflow} /></p></div><button className="rounded bg-active px-4 py-2 text-sm font-semibold text-white disabled:cursor-not-allowed disabled:opacity-50" disabled={!canRunReplay} onClick={() => void controller.analyze()} type="button">{workflow.phase === "ANALYZING" ? "Running replay…" : "Run replay"}</button></header><div className="mt-5 grid gap-4"><section className="rounded-lg border border-edge bg-raised p-4" aria-labelledby="inventory-title"><Label>Namespace inventory</Label><h3 id="inventory-title" className="mt-2 text-base font-semibold">{KUBERNETES_PUBLIC_REPLAY_SNAPSHOT.resources.length} captured offline resources</h3><ul className="mt-3 grid gap-2 text-sm">{KUBERNETES_PUBLIC_REPLAY_SNAPSHOT.resources.map((resource) => <li className="rounded border border-edge bg-canvas p-3" key={resource.resourceId}><p className="font-medium">{resourceName(resource)}</p><p className="mt-1 font-mono text-xs text-ink-faint">{resource.resourceId}</p>{resourceLabelEntries(resource).length ? <p className="mt-2 text-xs text-ink-dim">Labels: {resourceLabelEntries(resource).map(([key, value]) => `${key}=${value}`).join(", ")}</p> : null}</li>)}</ul></section><section className="overflow-hidden rounded-lg border border-edge bg-raised" aria-labelledby="selector-title"><div className="p-4"><Label>Service selector relationships</Label><h3 id="selector-title" className="mt-2 text-base font-semibold">Current snapshot relationships</h3></div><div className="overflow-x-auto"><table className="min-w-full border-collapse text-left text-xs"><thead className="border-y border-edge bg-surface text-ink-faint"><tr><th className="px-4 py-3 font-medium">Service</th><th className="px-4 py-3 font-medium">Selector</th><th className="px-4 py-3 font-medium">Matching workloads</th></tr></thead><tbody>{relations.map(({ service, selector, matches }) => <tr className="border-b border-edge align-top" key={service.resourceId}><td className="px-4 py-3 font-mono">{resourceName(service)}</td><td className="px-4 py-3">{Object.entries(selector).map(([key, value]) => `${key}=${value}`).join(", ") || "No selector"}</td><td className="px-4 py-3">{matches.length ? matches.map(resourceName).join(", ") : "No matching workload"}</td></tr>)}</tbody></table></div></section><section className="rounded-lg border border-edge bg-raised p-4" aria-labelledby="manifest-title"><Label>Current / proposed manifest diff</Label><h3 id="manifest-title" className="mt-2 text-base font-semibold">Bundled manifest proposal</h3><p className="mt-2 text-sm text-ink-dim">Image, workload security context, protected-resource labels, and Service selector effects are review evidence. Proposed manifests are data only and are never applied.</p><ProposedDiff proposal={fixture.proposal} /></section><section className="rounded-lg border border-edge bg-raised p-4" aria-labelledby="proposal-title"><Label>Evaluated proposal</Label><h3 id="proposal-title" className="mt-2 text-base font-semibold">Replay result only</h3><ProposalPanel state={workflow} /></section><section className="rounded-lg border border-edge bg-raised p-4" aria-labelledby="findings-title"><Label>Deterministic findings</Label><h3 id="findings-title" className="mt-2 text-base font-semibold">Image, security, selector, and protected-resource evidence</h3><FindingsPanel state={workflow} /></section><DomainCoverageCatalog catalog={coverageCatalog} evaluatedPolicyIds={hasFindings(workflow) ? workflow.findings.map((finding) => finding.policyId) : []} simulationDisclosure="Kubernetes has an offline sandbox capability, but this public replay never requests simulation because it has no decision authority." source={{ sourceId: selectedSourceId, source: example.session.source, analysisMode: example.session.analysisMode, provenance: example.session.provenance }} /></div></main>
      <aside aria-label="Kubernetes review authority" className="min-w-0 rounded-xl border border-edge bg-surface p-4"><Label>Airlock status</Label><section className="mt-4 border-t border-edge pt-4" aria-labelledby="risk-title"><h2 id="risk-title" className="text-sm font-semibold">Risk</h2><p className="mt-2 text-sm text-ink-dim">{hasFindings(workflow) ? workflow.riskLevel : "Not evaluated"}</p></section><section className="mt-4 border-t border-edge pt-4" aria-labelledby="decision-title"><h2 id="decision-title" className="text-sm font-semibold">Decision</h2><DecisionPanel state={workflow} /></section><section className="mt-4 border-t border-edge pt-4" aria-labelledby="simulation-title"><h2 id="simulation-title" className="text-sm font-semibold">Simulation</h2><p className="mt-2 text-sm text-ink-dim">Unavailable and not run. Kubernetes has an offline sandbox capability, but public replay is decision-free and this route provides no validated simulation result.</p></section><section className="mt-4 border-t border-edge pt-4" aria-labelledby="receipt-title"><h2 id="receipt-title" className="text-sm font-semibold">Receipt</h2><p className="mt-2 text-sm text-ink-dim">Not created. This ephemeral public replay has no durable decision or signed receipt.</p></section><section className="mt-4 border-t border-edge pt-4" aria-labelledby="execution-title"><h2 id="execution-title" className="text-sm font-semibold">Cluster contact or apply</h2><p className="mt-2 text-sm text-ink-dim">Not performed or observed. ChangeSafe never contacts this cluster or applies infrastructure changes.</p></section></aside>
    </div>
  </div>;
}
